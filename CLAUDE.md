...
### *arr internal config-as-code (TrueNAS)

`compose.yml` only defines the arr *containers*. Everything inside each app — indexers, custom formats, quality profiles, tags — lives in that app's SQLite DB under `/mnt/redsea/apps/arr/<app>/config/`, invisible to the compose reconciler and to drift detection. Two mechanisms bring it under version control; anything outside them is click-ops and **will** rot silently:

- **`servers/truenas/apps/recyclarr/`** — [Recyclarr](https://recyclarr.dev) syncs TRaSH-Guides custom formats, quality definitions and quality profiles into both Sonarrs and Radarr, daily at 04:00. Its config is inline in the app's `compose.yml` as a docker `config` targeted at `/config/recyclarr.yml` (works because `/config` already exists as a bind mount — the parent-dir problem that forces authentik blueprints out-of-band doesn't apply), so it *is* fingerprinted and drift-tracked. **Recyclarr 8 dropped the `includes/` mechanism** every v7-era example uses: `- template: sonarr-v4-custom-formats-anime` no longer resolves (the templates repo has no `includes/` dir; `includes.json` is an empty map). Profiles are referenced by TRaSH `trash_id`, the custom formats they need come with them, `custom_format_groups.add[].select[]` opts extras in, and `name:` overrides the guide's profile name — which is how existing profile names survive a first sync without migrating any series.
- **`servers/truenas/apps/arr/settings.yml`** + **`scripts/arr_settings_sync.py`** — declarative tags, indexer/download-client tag routing, auto-tagging rules, release profiles, `seriesType` enforcement and per-series tags. Plan by default, `--apply` to write, `--instance <name>` to scope. Idempotent; a clean run prints `= in sync`. Library state (series, queue, history, blocklist, files) is never touched.

**The tag routing model matters more than it looks.** `vpn`/`direct` are the routing mechanism: a tagged *indexer* is only searched for series carrying that tag, and a tagged *download client* is only eligible for series carrying it. Since both qBittorrent clients are tagged, an **untagged series has no eligible download client at all** and can never grab — while RSS sync (which ignores tags) keeps the Activity page looking busy. The `vpn-default` auto-tagging rule fixes this generally: `TagSpecification(direct)` with `negate: true`, plus `removeTagsAutomatically: true` for mutual exclusion. Note the autotagging API wants `specifications[].fields` as a **list** of `{name, value}` — a dict 400s.

**Anime-specific traps** (all five hit simultaneously on 2026-07-31, each looking like "Sonarr is broken"): `seriesType: standard` on an anime series makes searches use `S17E41` which no group names; untagged series as above; stale custom formats (TRaSH restructured the anime tiers — a hand-pushed copy left `ToonsHub`/`AnoZu` at score 0); `min_format_score: 100` combined with those stale formats means nothing clears the floor and interactive search just looks empty; and **cour-relative `SxxExx` numbering** — some groups ship Bleach's Thousand-Year Blood War as `S01E41` though TVDB puts it at `S17E41`, so Sonarr matches the release *title* (which carries the TYBW alias) but imports the *file* name, which drops it, and bare `Bleach S01E41` scene-maps onto the real season 1 (`Episode 2x21 was not found in the grabbed release`). Unresolvable in general — the `reject cour-relative SxxExx` release profile refuses them for tagged series. To diagnose: `GET /api/v3/parse?title=<release>` shows what Sonarr maps a title to; compare against the queue item's `outputPath`, which is what import actually parses.
...
Three rules that bite: (1) `homepage.widget.url` must be the **internal** service URL (`http://sonarr:8989`, container name on `proxy`) — never the Authentik-gated public host, or the server-side widget call 302s to login. Host-net apps (AdGuard, Home Assistant) have no container DNS, so they live in `config/services.yaml` with a `http://192.168.1.138:<port>` URL instead of labels. (2) Widget creds are `{{HOMEPAGE_VAR_*}}` placeholders resolved from **Homepage's own** env — so the real value goes in `homepage/.env` (vault), enumerated in homepage's `compose.yml` `environment:`, NOT the labelled app's `.env`. The arr keys + `AUTHENTIK_API_TOKEN` are reused (copied) from those apps' vaults; the rest are `replace-me` stubs (a stubbed widget just shows no data; the link tile still works). (3) `HOMEPAGE_ALLOWED_HOSTS=www.bajaber.ca` is mandatory behind Traefik.

Delivery mirrors AdGuard/blueprints — TrueNAS ships only the compose body, so `config/*.yaml` is pushed out-of-band (NOT drift-tracked): `python scripts/homepage_push_config.py`. `www.bajaber.ca` is an AdGuard rewrite to the box (internal/tailnet only) in `AdGuardHome.yaml` — re-push with `scripts/adguard_push_config.py`. Editing a `homepage.*` label on another app needs that app re-applied (`truenas_sync.yml -e mode=apply`); Homepage re-discovers on the next page load.

### Authentik config-as-code via blueprints (TrueNAS)

Authentik's internal config (sources, providers, applications, flows, groups, RBAC) lives in its **postgres DB**, not in this repo — the `authentik/` app folder only defines the container shape. To manage that config declaratively, use Authentik **blueprints** (its native YAML config-as-code; the worker reconciles them into the DB, idempotently). The first one shipped is `servers/truenas/apps/authentik/blueprints/maryam-google.yaml` (a Google social-login source). The exhaustive how-to (with the dead-ends we hit) is in the `authentik-blueprints-truenas` memory; the essentials:

- **Repo layout**: blueprint files in `servers/truenas/apps/authentik/blueprints/*.yaml`. The `authentik-worker` bind-mounts `/mnt/redsea/apps/authentik/blueprints → /blueprints/custom` (read-only) in `compose.yml`.
- **Extension MUST be `.yaml`** — Authentik's discovery silently ignores `.yml`. (This cost an afternoon.)
- **Use a bind mount, not a compose `configs:` mount.** docker `configs` do **not** auto-create parent dirs, so a config targeted at `/blueprints/custom/x.yaml` never materializes (the container still starts — silent). Bind mounts auto-create the dir, and it's Authentik's documented approach. (The existing `permissions` config works only because its target dir `/script` already exists.)
- **File delivery is out-of-band.** TrueNAS ships only the compose body via `app.update`, not repo files — so the reconciler creates the bind dir on `apply` (`apps:apps 770`) but does **not** copy the blueprint into it, and **does not track it for drift**. Push it with `open-terminal`: `./scripts/ot.py write /mnt/apps/authentik/blueprints/<name>.yaml < servers/truenas/apps/authentik/blueprints/<name>.yaml`.
- **Secrets**: reference them in the blueprint as `!Env VAR`, and add `VAR: ${VAR}` to the **worker** `environment:` in compose (resolved from the vault `.env` by the reconciler). Example: `AUTHENTIK_GOOGLE_CLIENT_ID` / `AUTHENTIK_GOOGLE_CLIENT_SECRET`.

**Deploy/edit workflow**: (1) write/edit the `.yaml` (+ bind mount and `!Env` wiring in compose if new); (2) `ansible-playbook playbooks/truenas_sync.yml -e mode=apply` if compose changed; (3) push the file with `ot.py write`; (4) trigger discovery — `GET /api/v3/tasks/schedules/?search=blueprints_discovery` then `POST /api/v3/tasks/schedules/<id>/send/` (no restart needed; the worker reads the live bind dir); (5) verify via `GET /api/v3/managed/blueprints/` (status `successful`) and the created object.

**Authentik API**: token is `AUTHENTIK_API_TOKEN` in `servers/truenas/apps/authentik/.env`; base `https://auth.bajaber.ca/api/v3/` with `Authorization: Bearer`. The `authentik` Traefik router has no forwardauth middleware, so the API is reachable directly. From the workstation, DNS won't resolve internally — use `curl --resolve auth.bajaber.ca:443:192.168.1.138 -k`. `scripts/authentik_explore.py` is a read-only inventory dump (users/groups/apps/providers/flows/outposts). The current task framework is `/api/v3/tasks/tasks/` + `/api/v3/tasks/schedules/` (legacy `admin/system_tasks/` is empty). The TrueNAS mgmt API (JSON-RPC) is reachable via host `truenas.bajaber.ca:4443`, **not** the LAN IP `192.168.1.183` (not routable from the workstation).
...
8. **Sidecar file delivery to TrueNAS apps.** Unlike the Docker VM (which rsyncs the whole app folder), the TrueNAS reconciler ships only the compose body — so any non-compose file an app needs on disk (Authentik blueprints, a config the container reads from a bind path, a seed file) must be pushed out-of-band via `open-terminal` and is **untracked for drift**. Compose `configs:` with inline `content` is the only in-repo channel, but it doesn't auto-create parent dirs and is awkward for multi-line bodies (see the blueprints section for why the Google source uses a bind mount + manual push instead). A proper fix: have the reconciler sync a declared set of files (e.g. an `app.yml` `files:` map, or everything under a conventional `files/` subdir) into the app's dataset on apply — via the filesystem JSON-RPC methods or by shelling `open-terminal` — and fold their content into the fingerprint so edits trigger drift. This would let blueprints (and similar) deploy through a single `apply` with no manual `ot.py write`.

Recommended order to tackle these when the need shows up: (1) → (2) → (4) → (8) → (3) → (7) → (6) → (5). Each is additive — no existing app needs to change when a new field becomes available.
